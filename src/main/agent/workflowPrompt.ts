...
export function buildStageInstructions(input: StageAgentInput): string {
  ...
  return [
    ...
    "allowed_tools 是当前阶段的能力边界（由工作流引擎注入到 SDK），不是平台限制：如果完成阶段需要的能力（如 git/构建/网络）不在 allowed_tools 中，请在 output_summary 中说明缺失能力并请求 needs_rework 到合适的阶段，或在该阶段限制下做替代方案；不要对用户说‘我没有 X 工具’让用户介入。",
