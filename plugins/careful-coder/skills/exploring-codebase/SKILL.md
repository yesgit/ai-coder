...
- never convert lexical similarity directly into `yes`; adjudicate every candidate returned in the current semantic review frontier. The host may prune its lower-ranked tail only after a reviewed frontier yields a `yes`, and must expand the next frontier when a batch is entirely rejected;
